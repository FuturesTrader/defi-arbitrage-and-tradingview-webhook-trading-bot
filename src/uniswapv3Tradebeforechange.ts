...
export async function executeUniswapTrade(tradeResult: TradeResult, network?: NetworkKey): Promise<TradeExecutionResult> {
    ...
    try {
        ...
        logger.info('Starting network-aware trade execution', {
            ...
            outputToken: trade.outputAmount.currency.symbol || 'UNKNOWN',
        ...
        const [balance, allowance] = await Promise.race([
            ...
            new Promise<never>((_, reject) =>
                setTimeout(() => reject(new Error('Balance/allowance check timeout')), timeoutMs)
            )
        ]);

        if ((balance as bigint) < inputAmount) {
            logger.error('Insufficient balance for trade on network', {
                network: targetNetwork,
                required: formatUnits(inputAmount, inputTokenDecimals),
                available: formatUnits(balance as bigint, inputTokenDecimals),
            ...
            return {
                state: TransactionState.Failed,
                error: `Insufficient balance on ${targetNetwork}: ${formatUnits(balance as bigint, inputTokenDecimals)} ${inputTokenSymbol} available, ${formatUnits(inputAmount, inputTokenDecimals)} required`
            };
        }

        // Handle approval if needed
        if ((allowance as bigint) < inputAmount) {
            logger.info('Insufficient allowance, approving token on network', {
                network: targetNetwork,
                token: inputTokenSymbol,
                spender: networkConfig.addresses.UNISWAP_V3.ROUTER,
                amount: 'unlimited',
                tradeId
            });

            const simulateApprovalPromise = publicClient.simulateContract({
                account: walletClient.account,
                address: inputTokenAddress,
                abi: ABIS.ERC20 as Abi,
                functionName: 'approve',
                args: [
                    networkConfig.addresses.UNISWAP_V3.ROUTER,
                    BigInt(2) ** BigInt(256) - BigInt(1)
                ]
            });

            const { request } = await Promise.race([
                simulateApprovalPromise,
                new Promise<never>((_, reject) =>
                    setTimeout(() => reject(new Error('Approval simulation timeout')), timeoutMs)
                )
            ]);

            const approvalHash = await walletClient.writeContract(request);
            logger.info('Approval transaction sent on network', {
                network: targetNetwork,
                hash: approvalHash,
                tradeId
            });

            // Wait for approval with network-specific timeout
            const approvalReceiptPromise = publicClient.waitForTransactionReceipt({ hash: approvalHash });
            const approvalTimeout = targetNetwork === 'ARBITRUM' ? 30000 : 60000; // Faster for L2

            const approvalReceipt = await Promise.race([
                approvalReceiptPromise,
                new Promise<never>((_, reject) =>
                    setTimeout(() => reject(new Error('Approval confirmation timeout')), approvalTimeout)
                )
            ]);

            logger.info('Approval transaction confirmed on network', {
                network: targetNetwork,
                hash: approvalHash,
        ...
        // Get pool fee for swap parameters
        let poolFee: number;
        try {
            poolFee = CurrentConfig?.tokens.poolFee || POOL_FEES.MEDIUM;
        } catch {
            poolFee = POOL_FEES.MEDIUM;
            logger.warn('Could not determine pool fee, using medium fee as fallback', {
                network: targetNetwork,
                fallbackFee: poolFee,
                tradeId
            });
        ...
        // Calculate minimum amount out with network-specific slippage
        ...
        const params = {
            ...
            fee: poolFee,
            recipient: walletClient.account?.address || '0x0',
            amountIn: inputAmount,
            amountOutMinimum: BigInt(minimumAmountOut.quotient.toString()),
            sqrtPriceLimitX96: BigInt(0)
        };

        logger.info('Executing swap on network with network-specific parameters', {
            network: targetNetwork,
            tokenIn: inputTokenSymbol,
            tokenOut: trade.outputAmount.currency.symbol || 'UNKNOWN',
            amountIn: formatUnits(inputAmount, inputTokenDecimals),
            minimumAmountOut: formatUnits(
                BigInt(minimumAmountOut.quotient.toString()),
                trade.outputAmount.currency.decimals
            ),
            slippage: `${networkConfig.tradeSettings.DEFAULT_SLIPPAGE_BPS / 100}%`,
            poolFee,
        ...
        logger.info('Network-aware swap transaction confirmed', {
            network: targetNetwork,
            hash,
