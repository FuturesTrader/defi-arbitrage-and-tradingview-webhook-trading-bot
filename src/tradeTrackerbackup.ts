...
export class TradeTracker {
    ...
    public async recordTrade(params: {
        ...
        try {
            ...
            const executionTimestamp = getCurrentTimestamp();
            const signalToExecutionDelayMs = (executionTimestamp - signalTimestamp) * 1000;

            // Format timestamps
            const signalTimestampCDT = formatCDTTimestamp(signalTimestamp);
            const executionTimestampCDT = formatCDTTimestamp(executionTimestamp);
            ...
            const trade: TradeEntry = {
                ...
                signalToExecutionDelayFormatted: this.formatDuration(signalToExecutionDelayMs),

                // Trade information (existing)
                entryPrice: undefined,
                entryAmount: params.tradeResult.actualAmountIn || '0',
                expectedOutput: params.tradeResult.expectedAmountOut || '0',
                actualOutput: this.extractActualOutput(params.tradeResult, params.webhookData),
                ...
                metadata: {
                    webhookReceived: signalTimestamp,
                    tradeExecuted: executionTimestamp,
    ...
    private async createCompletedTrade(tradeA: TradeEntry, tradeB: TradeEntry): Promise<CompletedTrade> {
        ...
        const signalDurationMs = secondTrade.signalTimestamp - firstTrade.signalTimestamp;
        ...
        const completedTrade: CompletedTrade = {
            ...
            netProfitUSDC,
            profitPercentage,
            expectedGrossProfitUSDC: grossProfitUSDC,
    ...
    private async attemptTradeMatching(): Promise<void> {
        try {
            ...
            for (const entryTrade of entryTrades) {
                const matchingExitTrade = exitTrades.find(
                    exit => exit.tokenPair === entryTrade.tokenPair &&
                        exit.signalTimestamp > entryTrade.signalTimestamp
                );

                if (matchingExitTrade) {
