...
export async function executeUniswapTrade(tradeResult: TradeResult, network?: NetworkKey, webhookId?: string): Promise<TradeExecutionResult> {
    ...
    try {
        ...
        // 🚨 NEW: Pre-flight nonce health check
        logger.info('🔍 Pre-flight nonce health check', {
            network: targetNetwork,
            tradeId,
            webhookId: webhookId || 'cli'
        });
        ...
        try {
            ...
            const isHealthy = await nonceManager.isHealthy();

            if (!isHealthy) {
                logger.warn('⚠️ Nonce manager unhealthy - performing reset before trade', {
                    network: targetNetwork,
                    tradeId,
                    webhookId: webhookId || 'cli',
                    account: walletClient.account.address
                ...
                await nonceManager.manualReset();

                logger.info('✅ Nonce manager reset completed - proceeding with trade', {
                    network: targetNetwork,
                    tradeId,
                    webhookId: webhookId || 'cli'
                });
            } else {
                logger.debug('✅ Nonce manager healthy - proceeding with trade', {
                    network: targetNetwork,
                    tradeId,
                    webhookId: webhookId || 'cli'
                });
            ...
            logger.warn('⚠️ Nonce health check failed - proceeding with caution', {
                network: targetNetwork,
                tradeId,
                webhookId: webhookId || 'cli',
                error: getErrorMessage(healthCheckError)
            });
            // Continue with trade execution - health check failure shouldn't block trades
        }

        logger.info('Starting network-aware trade execution with enhanced nonce management', {
            tradeId,
            network: targetNetwork,
        ...
        if (allowance < inputAmount) {
            logger.info('Approving token allowance for network with enhanced nonce management', {
                network: targetNetwork,
        ...
        logger.info('Executing swap on network with enhanced nonce management', {
            network: targetNetwork,
            router: routerAddress,
            tradeId,
            webhookId: webhookId || 'cli',
        ...
        // Execute swap with enhanced nonce management
        ...
        logger.info('Swap transaction sent on network with enhanced nonce management', {
            network: targetNetwork,
            hash,
            tradeId,
            webhookId: webhookId || 'cli'
        ...
        logger.info('Network-aware swap transaction confirmed with enhanced nonce management', {
            network: targetNetwork,
            hash,
        ...
        // 🚨 ENHANCED: Specific nonce error handling with new methods
        if (errorDetails.message.includes('nonce')) {

            logger.error('🚨 Nonce-related error detected - initiating enhanced recovery', {
                network: targetNetwork,
                tradeId,
                webhookId: webhookId || 'cli',
            ...
            try {
                if (!walletClient.account?.address) {
                    throw new Error('Wallet account not available for nonce recovery');
                }

                const nonceManager = getNetworkNonceManager(walletClient.account.address, targetNetwork);

                // 🔧 ENHANCED: Use specific error handlers instead of generic refreshNonce
                if (errorDetails.message.includes('higher than') || errorDetails.message.includes('too high')) {

                    logger.error('🚨 "Nonce too high" error - using specific recovery handler', {
                        network: targetNetwork,
                        tradeId,
                        webhookId: webhookId || 'cli',
                        error: errorDetails.message,
                        errorType: 'nonce_too_high',
                        recoveryMethod: 'handleNonceTooHighError'
                    });

                    // Extract failed nonce from error message if possible
                    const nonceMatch = errorDetails.message.match(/nonce.*?(\d+)/i);
                    const failedNonce = nonceMatch ? parseInt(nonceMatch[1]) : 0;

                    await nonceManager.handleNonceTooHighError(failedNonce, tradeId, webhookId);
                ...
                } else if (errorDetails.message.includes('lower than') || errorDetails.message.includes('too low')) {

                    logger.error('🚨 "Nonce too low" error - using specific recovery handler', {
                        network: targetNetwork,
                        tradeId,
                        webhookId: webhookId || 'cli',
                        error: errorDetails.message,
                        errorType: 'nonce_too_low',
                        recoveryMethod: 'handleNonceTooLowError'
                    });
                    ...
                    await nonceManager.handleNonceTooLowError(failedNonce, tradeId, webhookId);
                ...
                } else if (errorDetails.message.includes('replacement transaction underpriced')) {

                    logger.error('💰 Replacement transaction underpriced - gas price issue', {
                        network: targetNetwork,
                        tradeId,
                        webhookId: webhookId || 'cli',
                        error: errorDetails.message,
                        errorType: 'gas_price_too_low',
                        suggestion: 'Transaction may be stuck due to low gas price'
                    });

                    // For gas price issues, we might want a lighter touch recovery
                    await nonceManager.refreshNonce();

                    logger.info('🔄 Basic nonce refresh completed for gas price issue', {
                        network: targetNetwork,
                        account: walletClient.account.address,
                        webhookId: webhookId || 'cli'
                    });
                    ...
                    logger.error('🔍 Generic nonce error - using standard recovery', {
                        network: targetNetwork,
                        tradeId,
                        webhookId: webhookId || 'cli',
                        error: errorDetails.message,
                        errorType: 'nonce_generic',
                        recoveryMethod: 'refreshNonce'
                    });

                    // For generic nonce errors, use the standard refresh
                    await nonceManager.refreshNonce();

                    logger.info('🔄 Standard nonce refresh completed', {
                        network: targetNetwork,
                        account: walletClient.account.address,
                        webhookId: webhookId || 'cli'
                    });
                ...
                // 🚨 NEW: Post-recovery health check
                const postRecoveryHealthy = await nonceManager.isHealthy();
                logger.info('📊 Post-recovery nonce health status', {
                    network: targetNetwork,
                    healthy: postRecoveryHealthy,
                    tradeId,
                    webhookId: webhookId || 'cli'
                });

                if (!postRecoveryHealthy) {
                    logger.warn('⚠️ Nonce manager still unhealthy after recovery', {
                        network: targetNetwork,
                        tradeId,
                        webhookId: webhookId || 'cli',
                        suggestion: 'Consider manual reset or investigation'
                    });
                }

            } catch (recoveryError) {
                logger.error('❌ Enhanced nonce recovery failed', {
                    network: targetNetwork,
                    tradeId,
                    webhookId: webhookId || 'cli',
            ...
            // Non-nonce errors
            logger.error('Trade execution failed - non-nonce error', {
                network: targetNetwork,
                tradeId,
                webhookId: webhookId || 'cli',
                error: errorDetails.message,
        ...
        logger.error('Network-aware trade execution failed with enhanced nonce management', {
            network: targetNetwork,
            tradeId,
            webhookId: webhookId || 'cli',
            error: errorDetails.message,
            errorType: errorDetails.type,
            recoverable: errorDetails.recoverable,
            hasNonceRecovery: errorDetails.message.includes('nonce')
        });
